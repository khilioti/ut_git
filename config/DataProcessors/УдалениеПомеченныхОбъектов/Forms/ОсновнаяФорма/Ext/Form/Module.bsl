...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		Элементы.ГруппаПричины.Подсказка = " ";
	...
	Иначе
		...
		Элементы.ГруппаПричины.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ФоновоеЗаданиеЗапустить(Знач ПараметрыЗадания)
	...
	Если Не ПараметрыЗадания.ИскатьПомеченные Тогда
		...
		Если ПомеченныеНаУдаление.Количество() = 0 Тогда
			...
			Возврат Результат;
		КонецЕсли;
		ПараметрыЗадания.Вставить("ПомеченныеНаУдаление", ПомеченныеНаУдаление);
	КонецЕсли;
...
Процедура ФоновоеЗаданиеЗагрузитьРезультат(Результат)
	...
	Иначе
		...
		ПомеченныеНаУдаление = РезультатВыполненияВФоне.ПомеченныеНаУдаление;
		Для Каждого УдаляемыйСсылка Из ПомеченныеНаУдаление Цикл
