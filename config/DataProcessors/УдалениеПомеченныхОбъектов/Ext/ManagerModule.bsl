...
Процедура УдалитьПомеченныеОбъекты(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.Интерактивное
		И ПараметрыВыполнения.ПомеченныеНаУдаление.Количество() = 0 Тогда
	...
	КлиентОтметитьНачалоОбхода(ПараметрыВыполнения, "ПомеченныеНаУдаление");
...
Процедура КлиентОтметитьНачалоОбхода(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ПараметрыВыполнения.Свойство(ИмяКоллекции, Коллекция) Тогда
...
Процедура КлиентОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	ИначеЕсли ИмяКоллекции = "ВсеПомеченныеНаУдаление" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Анализ помеченных на удаление объектов: %1 из %2.'"),
...
Процедура ИнициализироватьПараметры(ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ВсеПомеченныеНаУдаление") Тогда
		ПараметрыВыполнения.Вставить("ВсеПомеченныеНаУдаление", Неопределено);
	КонецЕсли;
	...
	Если ПараметрыВыполнения.Интерактивное Тогда
		...
		Если ПараметрыВыполнения.ИскатьПомеченные И ПараметрыВыполнения.УдалятьПомеченные Тогда
			...
			ПараметрыВыполнения.Диапазоны.Вставить("ПомеченныеНаУдаление", 70);
			ПараметрыВыполнения.Диапазоны.Вставить("ПовторноУдаляемые", 10);
		ИначеЕсли ПараметрыВыполнения.УдалятьПомеченные Тогда
			ПараметрыВыполнения.Диапазоны.Вставить("ТехнологическиеОбъекты", 10);
			ПараметрыВыполнения.Диапазоны.Вставить("ПомеченныеНаУдаление", 80);
			ПараметрыВыполнения.Диапазоны.Вставить("ПовторноУдаляемые", 10);
		ИначеЕсли ПараметрыВыполнения.ИскатьПомеченные Тогда
			ПараметрыВыполнения.Диапазоны.Вставить("ПередПоискомПомеченныхНаУдаление", 30);
			ПараметрыВыполнения.Диапазоны.Вставить("ПоискПомеченныхНаУдаление", 40);
			ПараметрыВыполнения.Диапазоны.Вставить("ВсеПомеченныеНаУдаление", 30);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьПомеченныеНаУдаление(ПараметрыВыполнения) Экспорт
	...
	Для Каждого Ссылка Из ПараметрыВыполнения.ВсеПомеченныеНаУдаление Цикл
		Информация = СформироватьИнформациюОТипах(ПараметрыВыполнения, ТипЗнч(Ссылка));
