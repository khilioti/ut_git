...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ НовыйАдрес Тогда 
		ЗначениеРеквизитовПоКонтактнойИнформации(ЭтотОбъект, XDTOКонтактная);
	Иначе
		НаселенныйПунктДетально = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураЧастейАдресаНаселенногоПункта(ВидКонтактнойИнформации.ФорматАдреса);
	КонецЕсли;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаРоссия И НЕ НовыйАдрес Тогда
	...
	Если Не ЭтоВебСервис Тогда
		...
		Если XDTOКонтактная.Состав.Состав <> Неопределено И ТипЗнч(XDTOКонтактная.Состав.Состав) <> Тип("Строка") Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
				...
				РегионЗагружен = МодульАдресныйКлассификаторСлужебный.РегионЗагружен(XDTOКонтактная.Состав.Состав.СубъектРФ);
				Если РегионЗагружен = Ложь Тогда
...
Процедура УстановитьЗначениеИндекса(Контекст, XDTOКонтактная = Неопределено)
	...
	Адрес = Инфо.Состав;
	ИндексПоКлассификатору = Формат(УправлениеКонтактнойИнформациейСлужебный.ОпределитьПочтовыйИндексАдреса(Адрес), "ЧЦ=6; ЧРГ=' '; ЧН=; ЧГ=0");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "АдресныйКлассификаторАктуализирован" Тогда
		Если ЕстьКлассификатор И АдресныеСведенияПоРегионуДоступны(НаселенныйПунктДетально.Регион.Представление) Тогда
...
&НаСервереБезКонтекста
Функция АдресныеСведенияПоРегионуДоступны(НаименованиеСубъектаРФ)
	
	МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
	РегионЗагружен = МодульАдресныйКлассификаторСлужебный.РегионЗагружен(НаименованиеСубъектаРФ);
	
	Если РегионЗагружен <> Истина Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
