...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ПредставлениеУлицы) И ПолучениеДанныхСИспользованиемВебСервиса Тогда
		НаименованиеСокращение = УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(Параметры.ПредставлениеУлицы);
		Идентификатор = ПолучитьИдентификаторЗаписи(НаименованиеСокращение.Наименование);
		
		ПараметрыПоиска = Новый Структура;
		ПараметрыПоиска.Вставить("СкрыватьНеактуальные",              СкрыватьНеактуальныеАдреса);
		ПараметрыПоиска.Вставить("ФорматАдреса", ФорматАдреса);
		ПараметрыПоиска.Вставить("Сортировка",   "DESC");
		ПараметрыПоиска.Вставить("ПерваяЗапись", Идентификатор);
		ПараметрыПоиска.Вставить("РазмерПорции", 1);
		...
		Если ДанныеКлассификатора.Данные.Количество() > 0 Тогда
			Идентификатор = ДанныеКлассификатора.Данные[0].Идентификатор;
			ПараметрыПоиска.Вставить("ПерваяЗапись", Идентификатор);
		Иначе
	...
	Если ДанныеКлассификатора.Данные.Количество() = 0 Тогда
		...
		Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
		Элементы.ВариантыДополнительныхТерриторийВыбрать.АктивизироватьПоУмолчанию = Истина;
	КонецЕсли;
...
Процедура ВариантыАдресаПриАктивизацииСтроки(Элемент)
	Если ЕстьПодчиненныеЭлементы(Элемент.ТекущиеДанные.Идентификатор) Тогда
...
Процедура ПредыдущаяПорция(Команда)
	...
	ПараметрыПоиска.Вставить("ФорматАдреса", ФорматАдреса);
	ПараметрыПоиска.Вставить("РазмерПорции", РазмерПорции);
	ПараметрыПоиска.Вставить("ПерваяЗапись", ВариантыАдреса[0].Идентификатор);
	ПараметрыПоиска.Вставить("Сортировка",   "DESC");
...
Процедура СледующаяПорция(Команда)
	...
	ПараметрыПоиска.Вставить("ФорматАдреса", ФорматАдреса);
	ПараметрыПоиска.Вставить("РазмерПорции", РазмерПорции);
	ПараметрыПоиска.Вставить("ПерваяЗапись", ВариантыАдреса[ЧислоЗаписей - 1].Идентификатор);
	ПараметрыПоиска.Вставить("Сортировка",   "ASC");
...
&НаСервере
Функция ПолучитьИдентификаторЗаписи(Текст)
	...
	ДополнительныеПараметры.Вставить("РазмерПорции", 1);
	...
	Возврат ДанныеВыбора[0].Значение.Значение.Идентификатор;
...
Процедура ЗаполнитьПорциюВариантовПоПервойБукве(Текст)
	...
	ПараметрыПоиска.Вставить("ФорматАдреса", ФорматАдреса);
	ПараметрыПоиска.Вставить("РазмерПорции", РазмерПорции);
	ПараметрыПоиска.Вставить("ПерваяЗапись", ДанныеВыбора[0].Значение.Значение.Идентификатор);
	ПараметрыПоиска.Вставить("Сортировка",   "ASC");
