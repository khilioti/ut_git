...
Процедура ЗавершениеНастройкиОбменаЧерезВнешнееСоединение(Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Если ВыполнитьИнтерактивныйОбменДаннымиСейчас Тогда
		...
		Иначе
			ОткрытьФорму("Обработка.ПомощникИнтерактивногоОбменаДанными.Форма", ПараметрыФормы, , , ВариантОткрытияОкна.ОтдельноеОкно, , ,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Если ОткрытьПомощникСопоставления Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ОбменДаннымиXDTO") Тогда
			...
			Если МодульОбменДаннымиXDTOКлиентПовтИсп.ЭтоПланОбменаXDTO(Объект.УзелИнформационнойБазы) Тогда
				ИмяФормыПомощника = "Обработка.ПомощникИнтерактивногоОбменаДаннымиЧерезXDTO.Форма";
			КонецЕсли;
		...
		Иначе
			ОткрытьФорму(ИмяФормыПомощника, ПараметрыФормы, , , ВариантОткрытияОкна.ОтдельноеОкно, , ,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
...
Процедура СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации()
	...
	ПараметрыМетода = Новый Структура;
	ПараметрыМетода.Вставить("УзелИнформационнойБазы", Объект.УзелИнформационнойБазы);
	ПараметрыМетода.Вставить("ИдентификаторФайла", ИдентификаторФайлаСообщенияВСервисе);
	ПараметрыМетода.Вставить("ДатаНачалаОперации", ДатаНачалаОперации);
	ПараметрыМетода.Вставить("ПараметрыАутентификации", Объект.WSПароль);
	
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		УникальныйИдентификатор,
		"Обработки.ПомощникСозданияОбменаДанными.ВыполнитьЗагрузкуДанных",
		ПараметрыМетода,
		НСтр("ru = 'Загрузка справочников из сообщения обмена'"));
