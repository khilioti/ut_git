...
Процедура НачальноеЗаполнениеТабДок()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	КритерииПроверкиМагазиновРД_Сокращенные.Описание
	               |ПОМЕСТИТЬ ВТ
	               |ИЗ
	...
	Объект.ТабДок.Область("R8C4").Значение = КолвоКритериевМаг;
	Объект.ТабДок.Область("R8C5").Значение = "100 %";
...
Процедура ПроверитьХранилище()
	Если ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","ТЗзаполнения") <> Неопределено 
		ИЛИ ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Город") <> Неопределено
		ИЛИ ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Управляющий") <> Неопределено
		ИЛИ ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Подразделение") <> Неопределено
		ИЛИ ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","РД") <> Неопределено
		ИЛИ ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","НД") <> Неопределено ТОГДА
...
Процедура ПоместитьВХранилище(НомСтроки)
	Если НЕ ЗначениеЗаполнено(Объект.ВыбДокПроверки) Тогда
		ТЗ = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","ТЗзаполнения");
		...
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","ТЗзаполнения",ТЗ);
	КонецЕсли;
КонецПроцедуры

&НаСервере 
Процедура ОчиститьХранилище()
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","ТЗзаполнения",ПользователиКлиентСервер.ТекущийПользователь());
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","Город",ПользователиКлиентСервер.ТекущийПользователь());
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","Управляющий",ПользователиКлиентСервер.ТекущийПользователь());
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","Подразделение",ПользователиКлиентСервер.ТекущийПользователь());
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","РД",ПользователиКлиентСервер.ТекущийПользователь());
	ХранилищеНастроекДанныхФорм.Удалить("РД_Сокр","НД",ПользователиКлиентСервер.ТекущийПользователь());
КонецПроцедуры

&НаСервере
Процедура ВосстановитьИзХранилища()
	ПолеГород = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Город");
	Если ПолеГород <> Неопределено Тогда
		Объект.Город = ПолеГород;
	КонецЕсли;
	
	ПолеУправляющий = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Управляющий");
	Если ПолеУправляющий <> Неопределено Тогда
		Объект.Управляющий = ПолеУправляющий;
	КонецЕсли;
	
	ПолеПодразделение = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","Подразделение");
	Если ПолеПодразделение <> Неопределено Тогда
		Объект.Подразделение = ПолеПодразделение;
	КонецЕсли;
	
	ПолеРД = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","РД");
	Если ПолеРД <> Неопределено Тогда
		Объект.РД = ПолеРД;
	КонецЕсли;
	
	ПолеНД = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","НД");
	Если ПолеНД <> Неопределено Тогда
		Объект.НД = ПолеНД;
	КонецЕсли;
	
	ТЗзаполнения = ХранилищеНастроекДанныхФорм.Загрузить("РД_Сокр","ТЗзаполнения");
...
Процедура ПересчетСуммРазделов()
	...
	Для НомСтр = 13 по Объект.ТабДок.ВысотаТаблицы - 1 цикл
		...
		Если Родитель <>Неопределено Тогда
			...
			иначе
				ОбщСуммаОтвДеп = ОбщСуммаОтвДеп +  Число(Объект.ТабДок.Область("R" + Строка(НомСтр) + "C4").Значение);
			КонецЕсли;
			...
			Если РодительРодитель <> Неопределено И РодительРодитель.Текст <> "Сам" Тогда 
				Объект.ТабДок.Область("R" + РодительРодитель.Верх + "C4").Значение = Число(Объект.ТабДок.Область("R" + РодительРодитель.Верх + "C4").Значение) +  
					Число(Объект.ТабДок.Область("R" + Строка(НомСтр) + "C4").Значение);	
			КонецЕсли;
		КонецЕсли;
	...
	Объект.ТабДок.Область("R7C4").Значение	= ОбщСуммаМаг + ОбщСуммаОтвДеп;
	Если (КолвоКритериевМаг + КолвоКритериевОтвДеп) <> 0 Тогда
		Объект.ТабДок.Область("R7C5").Значение	= Строка(Формат(((ОбщСуммаМаг + ОбщСуммаОтвДеп)/(КолвоКритериевМаг + КолвоКритериевОтвДеп))*100,"ЧДЦ=2; ЧН=0,00"))+" %";
	Иначе 
		Объект.ТабДок.Область("R7C5").Значение	= Строка(Формат(0,"ЧДЦ=2; ЧН=0,00"))+" %";
	КонецЕсли;
...
Процедура ПересчетСуммРазделовДляЯчейки(НомСтр,Было,Стало)
	...
	//общий балл
	Объект.ТабДок.Область("R7C4").Значение	= Число(Объект.ТабДок.Область("R7C4").Значение)- Число(Было) + Число(Стало);
	Объект.ТабДок.Область("R7C5").Значение	= Строка(Формат((Объект.ТабДок.Область("R7C4").Значение/(КолвоКритериевМаг + КолвоКритериевОтвДеп))*100,"ЧДЦ=2; ЧН=0,00"))+" %";
	Если не ПризнакОтвДепертаменты тогда
		// балл магазина
...
Функция СохранитьВДокументНаСервере()
	...
	Если не ЗначениеЗаполнено(ВыбДокумент) тогда
		//создаем
		ТекДокПроверки = Документы.РезультатыПроверкиМагазинаРДСокращенные.СоздатьДокумент();
	...
	ТекДокПроверки.КолвоБаллов = ТабДок.Область("R7C4").Значение;
	ТекДокПроверки.ПроцентСоответствия = Лев(ТабДок.Область("R7C5").Значение,Найти(ТабДок.Область("R7C5").Значение,"%")-2);
...
Процедура ВыбДокПроверкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.РезультатыПроверкиМагазинаРДСокращенные.ФормаВыбора",ПараметрыВыбора,,,,, Новый ОписаниеОповещения("ВыбДокПроверкиНачалоВыбораЗавершение", ЭтотОбъект));	
КонецПроцедуры
...
Процедура ГородПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.Город) Тогда
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","Город",Объект.Город);
	КонецЕсли;
КонецПроцедуры
...
Процедура УправляющийПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.Управляющий) Тогда
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","Управляющий",Объект.Управляющий);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодразделениеПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","Подразделение",Объект.Подразделение);
	КонецЕсли;
КонецПроцедуры
...
Процедура РДПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.РД) Тогда
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","РД",Объект.РД);
	КонецЕсли;
КонецПроцедуры
...
Процедура НДПриИзмененииНаСервере()
	Если ЗначениеЗаполнено(Объект.НД) Тогда
		ХранилищеНастроекДанныхФорм.Сохранить("РД_Сокр","НД",Объект.НД);
	КонецЕсли;
КонецПроцедуры
