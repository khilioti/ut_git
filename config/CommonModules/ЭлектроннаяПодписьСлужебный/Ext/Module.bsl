...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	Менеджер = ЭлектроннаяПодписьСлужебныйКлиентСервер.МенеджерКриптографии(
		Программа, ЭтоLinux, Ошибки, Истина, ПутиКПрограммамНаСерверахLinux);
	...
	Если ЭлектроннаяПодпись.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
		...
		Если МенеджерКриптографии <> Неопределено Тогда
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ДобавитьСвойстваСертификатовИзХранилища(
				ТаблицаСвойствСертификатов, МенеджерКриптографии, Личные, БезОтбора);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеСвойстваСертификата(Форма)
	...
	ЭлектроннаяПодпись.ПолучитьСертификатПоОтпечатку(Форма.СертификатОтпечаток,
		Истина, Форма.СертификатПрограмма, Ложь, Форма.СертификатНаСервереОписаниеОшибки);
