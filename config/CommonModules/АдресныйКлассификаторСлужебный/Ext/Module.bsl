...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораСервис1С(Результат, Уровни, Родитель, Уровень, ДополнительныеПараметры);
	...
	Пока Истина Цикл
		Порция = Сервис.Select(РодительУровня, Уровень, ПерваяЗапись, "ASC", РазмерПорции, КодЯзыка, Метаданные.Имя);
		Результат.Заголовок = Порция.Title;
		Список = Порция.ПолучитьСписок("Item");
...
#Область ТекстЗапроса
			...
			|ВЫБРАТЬ ПЕРВЫЕ " + ОтбиратьЗаписей + "
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	РегионЗагруженВАдресныеСведения = РегионЗагружен(Адрес.СубъектРФ);
				...
				Если Не ПустаяСтрока(Описание) Тогда
					...
					Если Не ПустаяСтрока(Описание) Тогда
						...
						Если ОписаниеЗданийФИАССодержитДанные(Описание, ЗданияИПомещения) Тогда
							...
							// Проверить индекс
							Если ЗначениеЗаполнено(Индекс) И Формат(ВариантАдреса.Индекс, "ЧРГ=' '; ЧГ=0") <> Индекс Тогда
...
//
Функция ИндексАдресаПоЧастямАдреса(Адрес) Экспорт
...
Процедура УстановитьУровниАдресаПоЧастямАдреса(ЧастиАдреса, Уровни) Экспорт
	...
	Для каждого Уровень Из Уровни Цикл
		...
		ИначеЕсли Уровень = 90 Тогда
				СписокПолей.Вставить("ДополнительнаяТерритория");
				ТекстСоединения = ТекстСоединения + " ЛЕВОЕ СОЕДИНЕНИЕ ДополнительнаяТерритория КАК ДополнительнаяТерритория
					| ПО ДополнительнаяТерритория.КодСубъектаРФ =СубъектРФ.КодСубъектаРФ
					| И  ДополнительнаяТерритория.КодОкруга = " + КодОкруга + "
					| И  ДополнительнаяТерритория.КодРайона = " + КодРайона + "
					| И  ДополнительнаяТерритория.КодГорода = " + КодГорода + "
					| И  ДополнительнаяТерритория.КодВнутригородскогоРайона = " + КодВнутригородскогоРайона + "
					| И  ДополнительнаяТерритория.КодНаселенногоПункта = " + КодНаселенногоПункта + "
					| И  ДополнительнаяТерритория.КодУлицы = " + КодУлицы;
				КодДополнительнойТерритории = "ДополнительнаяТерритория.КодДополнительногоЭлемента";
		ИначеЕсли Уровень = 91 Тогда
			СписокПолей.Вставить("ЭлементДополнительнойТерритории");
			ТекстСоединения = ТекстСоединения + " ЛЕВОЕ СОЕДИНЕНИЕ ЭлементДополнительнойТерритории КАК ЭлементДополнительнойТерритории
				| ПО ЭлементДополнительнойТерритории.КодСубъектаРФ = СубъектРФ.КодСубъектаРФ
				| И  ЭлементДополнительнойТерритории.КодОкруга = " + КодОкруга + "
				| И  ЭлементДополнительнойТерритории.КодРайона = " + КодРайона + "
				| И  ЭлементДополнительнойТерритории.КодГорода = " + КодГорода + "
				| И  ЭлементДополнительнойТерритории.КодВнутригородскогоРайона = " + КодВнутригородскогоРайона + "
				| И  ЭлементДополнительнойТерритории.КодНаселенногоПункта = " + КодНаселенногоПункта + "
				| И  ЭлементДополнительнойТерритории.КодУлицы = " + КодУлицы + "
				| И  ЭлементДополнительнойТерритории.КодДополнительногоЭлемента = " + КодДополнительнойТерритории;
		КонецЕсли;
	КонецЦикла;
...
#Область СлужебныйПрограммныйИнтерфейсПреобразованиеТиповДанныхФИАС
...
// Проверяет наличие загруженных адресных сведений по региону.
//
Функция РегионЗагружен(НаименованиеРегиона) Экспорт
	...
	Если РезультатЗапроса.Следующий() Тогда
		Возврат РезультатЗапроса.РегионЗагружен;
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	//6. Удаляет устаревшие данные КЛАДР если они есть
...
Функция СлужебныеАдресныеСведения(КодСубъектаРФ, КаталогФайлов)
	...
	Пока СледующаяЗаписьФайлаЗагрузкиСубъектаРФ(ФайлЗагрузки, ТекущаяЗапись) Цикл
		Если ВРег(ТекущаяЗапись.TYPE) <> "SOCRBASE" Тогда
...
Процедура ЗагрузитьПричиныИзмененияАдреса(КодСубъектаРФ, КаталогФайлов, ВидыСведений)
	...
	ОбновлениеИнформационнойБазы.ЗаписатьДанные(Набор); // TOOD: Набор.Записать(Истина);
	ЗакрытьФайлЗагрузкиСубъектаРФ(ФайлЗагрузки);
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	// Как архив
	Файл = АдресныйКлассификаторКлиентСервер.НайтиФайл(КаталогФайлов, ИмяИсходногоФайла + ".ZIP");
...
Процедура ПеренестиРегионИзДанныхКЛАДР(СубъектРФ)
	...
	ЗаполнитьЗаполнитьДомаЗданияСтроенияКЛАДР(СубъектРФ, ТаблицыДанных);
...
Процедура ЗаполнитьЗаполнитьДомаЗданияСтроенияКЛАДР(СубъектРФ, ТаблицыДанных)
