...
Функция ПредставлениеИздателя(Сертификат) Экспорт
	...
	Если ЗначениеЗаполнено(Издатель.ОбщееИмя)
	   ...
	   И Издатель.Организация <> Издатель.ОбщееИмя Тогда
...
Функция ЗаполнитьСтруктуруСертификата(Сертификат) Экспорт
	...
	Свойства.Вставить("Представление", ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(
		ПредставлениеСертификата(Сертификат)));
	
	Свойства.Вставить("КомуВыдан",ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(
		ПредставлениеСубъекта(Сертификат)));
	
	СвойстваИздателя = СвойстваИздателяСертификата(Сертификат);
	
	Свойства.Вставить("КемВыдан", ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(
		ПредставлениеИздателя(Сертификат)));
		
	Свойства.Вставить("ДействителенДо", Сертификат.ДатаОкончания);
	
	Если Сертификат.РасширенныеСвойства.Свойство("EKU") Тогда
		Свойства.Вставить("Назначение", ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(
			ПолучитьНазначение(Сертификат.РасширенныеСвойства.EKU)));
	Иначе
		Свойства.Вставить("Назначение", "");
	КонецЕсли;
...
Функция СвойстваСубъектаСертификата(Сертификат) Экспорт
	...
	Если Субъект.Свойство("C") Тогда
		Свойства.Страна = СокрЛП(Субъект.C);
	КонецЕсли;
	
	Если Субъект.Свойство("ST") Тогда
		Свойства.Регион = СокрЛП(Субъект.ST);
	КонецЕсли;
	
	Если Субъект.Свойство("L") Тогда
		Свойства.НаселенныйПункт = СокрЛП(Субъект.L);
	КонецЕсли;
	
	Если Субъект.Свойство("Street") Тогда
		Свойства.Улица = СокрЛП(Субъект.Street);
	КонецЕсли;
	...
	ИначеЕсли Субъект.Свойство("OGRNIP") // Признак индивидуального предпринимателя.
		...
		Если Свойства.ОбщееИмя <> Свойства.Организация
		   И Не (Субъект.Свойство("T") И Свойства.ОбщееИмя = Субъект.T) Тогда
...
Функция СвойстваИздателяСертификата(Сертификат) Экспорт
	...
	Если Издатель.Свойство("CN") Тогда
		Свойства.ОбщееИмя = СокрЛП(Издатель.CN);
	КонецЕсли;
	
	Если Издатель.Свойство("C") Тогда
		Свойства.Страна = СокрЛП(Издатель.C);
	КонецЕсли;
	
	Если Издатель.Свойство("ST") Тогда
		Свойства.Регион = СокрЛП(Издатель.ST);
	КонецЕсли;
	
	Если Издатель.Свойство("L") Тогда
		Свойства.НаселенныйПункт = СокрЛП(Издатель.L);
	КонецЕсли;
	
	Если Издатель.Свойство("Street") Тогда
		Свойства.Улица = СокрЛП(Издатель.Street);
	КонецЕсли;
	
	Если Издатель.Свойство("O") Тогда
		Свойства.Организация = СокрЛП(Издатель.O);
	КонецЕсли;
	
	Если Издатель.Свойство("OU") Тогда
		Свойства.Подразделение = СокрЛП(Издатель.OU);
	КонецЕсли;
	
	Если Издатель.Свойство("E") Тогда
		Свойства.ЭлектроннаяПочта = СокрЛП(Издатель.E);
	КонецЕсли;
	
	Если Издатель.Свойство("OGRN") Тогда
		Свойства.ОГРН = СокрЛП(Издатель.OGRN);
	КонецЕсли;
	
	Если Издатель.Свойство("INN") Тогда
		Свойства.ИНН = СокрЛП(Издатель.INN);
	КонецЕсли;
