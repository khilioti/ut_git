...
Процедура ПередЗапускомПрограммы()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.5.1068") < 0 Тогда
		ВызватьИсключение НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие 8.3.5.1068 или выше.'");
	КонецЕсли;
