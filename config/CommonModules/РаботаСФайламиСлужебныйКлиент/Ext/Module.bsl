...
//
// Параметры:
//   Параметры - Структура - см. ПараметрыПубликацииФайла.
//
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиНовойВерсии(ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		...
		ПараметрыВыполнения.Вставить("МенеджерКриптографии",
			МодульЭлектроннаяПодписьКлиент.МенеджерКриптографии("Шифрование"));
		...
		Для Каждого СтруктураСертификата Из ПараметрыВыполнения.ДанныеФайла.МассивСертификатовШифрования Цикл
			...
			Иначе
				Сертификат = МодульЭлектроннаяПодписьКлиент.ПолучитьСертификатПоОтпечатку(
					СтруктураСертификата.Отпечаток, Ложь);
			КонецЕсли;
		...
		Если ПараметрыВыполнения.МассивСертификатов.Количество() = 0 Тогда
			...
			ЗакончитьРедактированиеСРасширениемПослеВыбораСертификатовДляШифрования(, ПараметрыВыполнения);
			Возврат;
		КонецЕсли;
...
// Продолжение процедуры (см. выше).
Процедура ЗакончитьРедактированиеСРасширениемПослеВыбораСертификатовДляШифрования(ОписаниеСертификатов, ПараметрыВыполнения) Экспорт
	
	Если ТипЗнч(ОписаниеСертификатов) <> Тип("Структура") Тогда
		ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, Ложь);
		Возврат;
	КонецЕсли;
	
	ПараметрыВыполнения.МенеджерКриптографии = ОписаниеСертификатов.МенеджерКриптографии;
	ПараметрыВыполнения.МассивСертификатов   = ОписаниеСертификатов.МассивСертификатов;
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(ПараметрыВыполнения)
	...
	Попытка
		ВерсияОбновлена = РаботаСФайламиСлужебныйВызовСервера.ОпубликоватьИОсвободитьФайл(ПараметрыВыполнения.ДанныеФайла, СведенияОФайле, 
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(ПомещенныеФайлы, ПараметрыВыполнения) Экспорт
	...
	СведенияОФайле.АдресВременногоХранилища = ОписаниеПереданногоФайла.Хранение;
	СведенияОФайле.Комментарий = ПараметрыВыполнения.КомментарийКВерсии;
	СтрокиПути = ОбщегоНазначенияКлиентСервер.РазложитьСтрокуПоТочкамИСлэшам(ОписаниеПереданногоФайла.Имя);
	Если СтрокиПути.Количество() >= 2 Тогда
		СведенияОФайле.РасширениеБезТочки = СтрокиПути[СтрокиПути.Количество() - 1];
		СведенияОФайле.ИмяБезРасширения = СтрокиПути[СтрокиПути.Количество() - 2];
	КонецЕсли;
	СведенияОФайле.ХранитьВерсии = ПараметрыВыполнения.СоздатьНовуюВерсию;
	
	Попытка
		Результат = РаботаСФайламиСлужебныйВызовСервера.ОпубликоватьИОсвободитьФайлПоСсылке(ПараметрыВыполнения.ОбъектСсылка,
...
Процедура ОбновитьИзФайлаНаДискеПослеПолученияФайлаВРабочийКаталог(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.РедактируетТекущийПользователь Тогда // Файл уже был занят.
		ПараметрыОбработчика = ПараметрыПубликацииФайла(Обработчик, ПараметрыВыполнения.ДанныеФайла.Ссылка, ПараметрыВыполнения.ИдентификаторФормы);
		...
		ОпубликоватьФайл(ПараметрыОбработчика);
	Иначе
		ПараметрыОбработчика = ПараметрыПубликацииФайла(Обработчик, ПараметрыВыполнения.ДанныеФайла.Ссылка, ПараметрыВыполнения.ИдентификаторФормы);
...
Процедура ОпубликоватьФайлыПоСсылкамПослеПомещенияФайлаНаСервер(Результат, ПараметрыВыполнения) Экспорт
	...
	Для Каждого Данные Из ПараметрыВыполнения.ДанныеФайлов Цикл
		...
		ПараметрыОбработчика = ПараметрыПубликацииФайла(Неопределено, Данные.Ссылка, ПараметрыВыполнения.ИдентификаторФормы); 
...
Функция ПараметрыПубликацииФайла(ОбработчикРезультата, ОбъектСсылка, ИдентификаторФормы) Экспорт
...
//
// Параметры:
//   ПараметрыПубликацииФайла - Структура - см. ПараметрыПубликацииФайла.
//
Процедура ОпубликоватьФайл(ПараметрыПубликацииФайла) 
	
	Обработчик = Новый ОписаниеОповещения("ОпубликоватьФайлПослеУстановкиРасширения", ЭтотОбъект, ПараметрыПубликацииФайла);
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик);
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		...
		Если ПараметрыВыполнения.МассивСертификатов.Количество() = 0 Тогда 
			ПоказатьПредупреждение(,
				НСтр("ru = 'У зашифрованного файла не указаны сертификаты.
				           |Расшифруйте файл и зашифруйте заново.'"));
			ОпубликоватьФайлСРасширениемПослеВыбораСертификатовДляШифрования(, ПараметрыВыполнения);
			Возврат;
		КонецЕсли;
...
// Продолжение процедуры (см. выше).
Процедура ОпубликоватьФайлСРасширениемПослеВыбораСертификатовДляШифрования(ОписаниеСертификатов, ПараметрыВыполнения) Экспорт
...
Процедура ОпубликоватьФайлСРасширениемПослеПроверкиЗашифрован(ПараметрыВыполнения)
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(СведенияОФайле.Размер / (1024 * 1024)));
	Состояние(ТекстПояснения);
	...
	ВерсияОбновлена = РаботаСФайламиСлужебныйВызовСервера.ОпубликоватьФайл(ПараметрыВыполнения.ДанныеФайла.Ссылка, СведенияОФайле, 
...
Процедура ОпубликоватьФайлБезРасширенияПослеЗагрузкиФайла(ПомещенныеФайлы, ПараметрыВыполнения) Экспорт
	...
	СведенияОФайле.ХранитьВерсии = ОписаниеПереданногоФайла.СоздатьНовуюВерсию;
	...
	Результат = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИОпубликоватьФайл(ПараметрыВыполнения.ОбъектСсылка, СведенияОФайле, 
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, ПолноеИмяФайла);
...
Процедура СохранитьКакБезРасширения(ПараметрыВыполнения) Экспорт
	...
	ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, ИмяФайла);
...
Процедура ЗакончитьРедактированиеСОповещением(Параметры) Экспорт
	...
	ПараметрыОбработчика = ПараметрыПубликацииФайла(Обработчик, Параметры.ОбъектСсылка, Параметры.ИдентификаторФормы);
...
Процедура ОпубликоватьФайлСОповещением(ОбработчикРезультата, ПараметрКоманды, ИдентификаторФормы) Экспорт
	...
	Иначе
		...
		ПараметрыОбработчика = ПараметрыПубликацииФайла(Обработчик, ПараметрКоманды, ИдентификаторФормы);
		ОпубликоватьФайл(ПараметрыОбработчика);
	КонецЕсли;
...
Процедура ПриПолученииЗашифрованныхДанных(ОписаниеДанных, ТекущиеПараметрыВыполнения) Экспорт
	
	ПараметрыВыполнения = ТекущиеПараметрыВыполнения.ПараметрыВыполнения;
	
	ЗашифрованныеДанные = ОписаниеДанных.ТекущийЭлементНабораДанных.ЗашифрованныеДанные;
...
Процедура ПриПолученииРасшифрованныхДанных(ОписаниеДанных, ТекущиеПараметрыВыполнения) Экспорт
	
	ПараметрыВыполнения = ТекущиеПараметрыВыполнения.ПараметрыВыполнения;
	
	РасшифрованныеДанные = ОписаниеДанных.ТекущийЭлементНабораДанных.РасшифрованныеДанные;
...
// Продолжение процедуры ПодписатьФайл, СохранитьФайлВместеСПодписями.
// Вызывается из подсистемы ЭлектроннаяПодпись при запросе данных для подписания.
//
Процедура ПриЗапросеДвоичныхДанныхФайла(ДвоичныеДанные, ПараметрыВыполнения) Экспорт
	
	ДвоичныеДанные = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаИДвоичныеДанные(
		ПараметрыВыполнения.ДанныеФайла.Ссылка).ДвоичныеДанные;
...
//
Процедура ПриПолученииПодписи(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	
	РаботаСФайламиСлужебныйВызовСервера.ДобавитьПодписьКФайлу(
		ПараметрыВыполнения.ДанныеФайла.Ссылка,
		ОписаниеДанных.ТекущийЭлементНабораДанных.СвойстваПодписи,
		ПараметрыВыполнения.ИдентификаторФормы);
...
// способа добавления подписи в объект.
//
Процедура ПриПолученииПодписей(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	
	РаботаСФайламиСлужебныйВызовСервера.ДобавитьПодписьКФайлу(
		ПараметрыВыполнения.ДанныеФайла.Ссылка,
		ОписаниеДанных.Подписи,
		ПараметрыВыполнения.ИдентификаторФормы);
...
//
Процедура ПриСохраненииДанныхФайла(ОбработкаСохраненияПодписей, ПараметрыВыполнения) Экспорт
	
	ПараметрыВыполнения.Вставить("ОбработкаСохраненияПодписей", ОбработкаСохраненияПодписей);
	
	СохранитьКак(ОбработкаСохраненияПодписей, ПараметрыВыполнения.ДанныеФайла,
		ПараметрыВыполнения.ИдентификаторФормы);
...
Процедура ЗаписатьСертификатыВТабличнуюЧасть(ФайлСсылка, МассивСертификатов, УникальныйИдентификатор)
	...
	Для Каждого Сертификат Из МассивСертификатов Цикл
		...
		Представление = МодульЭлектроннаяПодписьКлиентСервер.ПредставлениеСубъекта(Сертификат);
