...
//                          получения менеджера криптографии, когда он не указан, вызывается исключение).
// 
// Возвращаемое значение:
...
Функция ПроверитьСертификат(МенеджерКриптографии, Сертификат, ОписаниеОшибки) Экспорт
	...
	Если МенеджерКриптографииДляПроверки = Неопределено Тогда
		МенеджерКриптографииДляПроверки = МенеджерКриптографии("ПроверкаСертификата", Ложь, ОписаниеОшибки);
	КонецЕсли;
...
//     * ВыполнятьШифрованиеНаСервере         - Булево - если Истина то шифрование выполняется на сервере.
