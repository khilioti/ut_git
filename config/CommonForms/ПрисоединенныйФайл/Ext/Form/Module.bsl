...
Процедура ПриПолученииПодписи(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	ДобавитьПодписьКФайлуНаСервере(ОписаниеДанных.СвойстваПодписи);
...
Процедура ПриПолученииПодписей(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	ДобавитьПодписьКФайлуНаСервере(ОписаниеДанных.Подписи);
...
Процедура ПрочитатьЭП()
	...
	ОбщиеНастройки = МодульЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки();
	...
	Попытка
		МенеджерКриптографии =
			МодульЭлектроннаяПодписьКлиент.МенеджерКриптографии("ПолучениеСертификатов");
	Исключение
		Возврат;
	КонецПопытки;
	...
	Для каждого СтрокаТаблицы Из ЭлектронныеПодписи Цикл
		...
		Если ПустаяСтрока(СтрокаТаблицы.Отпечаток) Тогда // Подпись не была прочитана при записи объекта.
			...
			Если ЗначениеЗаполнено(Подпись) Тогда 
				...
				СтрокаТаблицы.КомуВыданСертификат =
					МодульЭлектроннаяПодписьКлиентСервер.ПредставлениеСубъекта(Сертификат);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
