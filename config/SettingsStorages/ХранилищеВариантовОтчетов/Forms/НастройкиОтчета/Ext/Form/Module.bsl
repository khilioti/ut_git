...
Функция ТаблицыПолейОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Для Каждого СтрокаИдентификатор Из ВыделенныеСтроки Цикл
		...
		Если Сценарий.ТаблицаИмя = "Отборы" Тогда
			...
			ИначеЕсли СтрокаДерева.ЭтоПараметр Тогда
				Если Сценарий.Действие = "Переместить" Тогда
					Сценарий.ТекстОшибки = СтрЗаменить(НСтр("ru = 'Параметры не могут быть перемещены.'"), "%1", СтрокаДерева.Представление);
...
Процедура ОтборыЗавершениеВыбораВидаСравнения(Результат, Контекст) Экспорт
	...
	СталВводСписком = (СтрокаТаблицы.Условие = ВидСравненияКомпоновкиДанных.ВСписке
		...
		Или СтрокаТаблицы.Условие = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии
	);
...
Процедура БыстрыеНастройкиЗапомнитьСостояние(ПараметрыЗаполнения)
	...
	ИменаТаблиц = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("Отборы, ВыбранныеПоля, Сортировка, СтруктураВарианта", ",", Истина, Истина);
...
Процедура ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	КонецЕсли;
	Если НаборСтрок = Неопределено Тогда
		НаборСтрок = ВыбранныеПоля.ПолучитьЭлементы();
	КонецЕсли;
