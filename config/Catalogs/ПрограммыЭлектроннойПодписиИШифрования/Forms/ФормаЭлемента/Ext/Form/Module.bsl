...
Процедура ЗаполнитьАлгоритмыВыбраннойПрограммы()
	...
	Попытка
		Менеджер = Новый МенеджерКриптографии(Объект.ИмяПрограммы, ПутьКПрограмме, Объект.ТипПрограммы);
		ИнформацияМодуля = Менеджер.ПолучитьИнформациюМодуляКриптографии();
		Если Объект.ИмяПрограммы <> ИнформацияМодуля.Имя И ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
			ИнформацияМодуля = Неопределено;
		КонецЕсли;
	Исключение
		// Если менеджер криптографии не получен и не из числа поставляемых,
