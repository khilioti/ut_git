...
Процедура ОбновлениеИзФайлаМеханикаНаСервере(ПараметрыРегистрации)
	...
	Иначе
		...
		КонецЕсли;
		СтандартныеПодсистемыКлиентСервер.РезультатВыполненияДобавитьПредупреждение(
			ПараметрыРегистрации,
			ПараметрыРегистрации.ТекстОшибки,
			НСтр("ru = 'Техническая информация:'") + Символы.ПС + ПараметрыРегистрации.КраткоеПредставлениеОшибки,
			ЗаголовокОшибки);
	КонецЕсли;
